...
from Wav2vec import Wav2Vec_SER
from data_prepare import extract_emotion, load_and_cut_audio
import hashlib
...
CACHE_DIR = r"D:/dataset/iemocap_cache(wav2vec)"
MODEL_DIR = "./models/model4(wav2vec)"
os.makedirs(CACHE_DIR, exist_ok=True)
os.makedirs(MODEL_DIR, exist_ok=True)
EMOTION_LABELS = ["angry", "happy", "sad", "neutral"]
local_path = "./wav2vec2-base-960h"

processor = Wav2Vec2Processor.from_pretrained(local_path)
wav2vec2_model = Wav2Vec2Model.from_pretrained(local_path)
wav2vec2_model.eval()
...
class IEMOCAPDataset(Dataset):
    def __init__(self, emotions=EMOTION_LABELS):
        ...
        for i, (wav_path, start, end) in enumerate(
                tqdm(self.utts, desc="Caching Wav2Vec2 features", unit="utt", leave=True)
        ):
...
def evaluate(model, dataloader, device, fold_epoch):
    ...
    if fold_epoch % 20 == 0:
        ...
        if fold_epoch == 60:
            plt.savefig(f'{MODEL_DIR}/wav2vec2_cm.png')
        plt.show()
    return ua_acc, acc


def train_with_wav2vec(num_epochs, batch_size, lr):
    ...
    # 初始化模型
    model = Wav2Vec_SER(num_classes=len(EMOTION_LABELS)).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_path = f"{MODEL_DIR}/best_wav2vec_model.pth"
            torch.save({
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'best_val_acc': best_val_acc,
                'best_ua_acc': best_ua_acc
            }, model_path)
    ...
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.title('Wav2Vec2-Based Training Loss')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()
    plt.savefig(f'{MODEL_DIR}/wav2vec_loss.png')
    plt.show()
    ...
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.title('Wav2Vec2-Based Training and Validation Accuracy')
    plt.ylim(0, 1)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{MODEL_DIR}/wav2vec_acc.png')
    plt.show()
    ...
    result_file = os.path.join(MODEL_DIR, "wav2vec_best_results.txt")
    ...
    print(f"Wav2Vec2-based model results saved to {result_file}")

    return final_val_acc, (train_loss_history, train_acc_history, val_acc_history)


if __name__ == '__main__':
    train_with_wav2vec(num_epochs=100, batch_size=16, lr=1e-3)
